...
subtitle: "Wham! GAM! Thank you, mam! Generalised additive models"
...
* [Case study: Portugese larks](#larks)
...
At the time of posting these slides, Amin and I were still finishing up the rubric for the professional development task, but will be aiming to post it Tuesday. As said before, the main things you need to know were described in last Monday's announcemnt.
...
- Let's get even more flexible!
    - What are GAMs?
    - Flexibly modelleing non-linear behaviour
]
...
```{r echo=FALSE}
p + geom_smooth(method="gam", color="purple", formula = y ~ s(x, k = 8))
```
...
GAMs are both *smooth* and *flexible* thanks to actually being made up of mutliple not as flexible functions. _Imagine the Power Rangers robot teaching you a yoga class._
...
There are LOTS of ways to pick your wiggle: AIC, generalized crossvalidation (GCV, part of the name of `mgcv` and the default for that package), ML and REML. 
...
So! If the __effective degrees of freedom__ we need for a term is approximately 1, then we're really just smoothing it down to a linear term, the way a covariate would usually enter a model as a fixed effect in our previous moodels. We may choose to just put it in the model as such, so that we can interpret the cofficient it receives. 
...
If your random effects are fairly simple, you can fit those in `mgcs::gam()` without needing the more complex GAMM functions, like `gamm4::gamm4()`
...
How accurate your predictions are depends on how good the model is, as always. (Credit: Erik Peterson & Gavin Newsome)

```{r misspecify, echo = FALSE, fig.height=2.3, fig.width=6}
set.seed(15)
...
If you don't have this, smoothness selection can usually only penalise a smooth back to a linear function]

.pull-right[ (because the penalty that's doing smoothness selection only works on the non-smooth (the wiggly) parts of the basis). To perform selection we need to be able to penalise the null space (the smooth parts of the basis) as well.

#### Smoothness selection
The `method` argument to gam selects the smoothness selection criterion. For many practioners, 'ML' or 'REML' are their default choice, though not the default in `gam()`. Using a likelihood based approach essentially treats the smooth components as random effects.
]]
...
GAMs are especially useful for accounting for a non-linear phenomonon that may not be the main thing you are interested in. This is similar to how we have already used random effects to account for correlation in our data that is not the main thing of interest but shouldn't be ignored.
