...
include_graphics("images/w1/uncleben.gif")
```
...
include_graphics("images/w1/wilcoxon.png")
```
...
What R is doing is dropping the first level (alphabetically) of the categorical variable and then create **dummy variables** for each of the other levels.

The dropped level becomes our reference level and this should be familiar from interpreting summary output for multiple linear regressions you have performed with categorical variables. 

A dummy variable is also called an indicator variable, and it *indicates* whether or not the given observation takes that level or not.

The sum across the row of the dummy variables for one categorical variable will either be 0 (in the reference level) or 1 (not in the reference level) but you will never have more than one 'one' amongst the dummies for a certain categorical variable.
...
Credit to **Jonas Kristoffer Lindeløv** for the excellent resource this section is based on. [There are more examples there than we will cover in this course.](https://lindeloev.github.io/tests-as-linear/)
